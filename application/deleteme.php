...
class ajax_contractors extends CI_Controller {
    ...
    public function submit_new_account()
    {
        ...
        $this->form_validation->set_rules('email', 'Email', 'required|min_length[3]|max_length[60]|valid_email|xss_clean|is_unique[contractor.user]|is_unique[contractor.email]');
        ...
        $this->form_validation->set_rules('category', 'Category', 'min_length[2]|max_length[100]|xss_clean|required');
        ...
        if ($this->form_validation->run() !== true) {
            ...
            if($this->process_payment->total > 0) {
                ...
                echo json_encode($creditCardDetails);
                exit;
                $test = true;
                if($test != true){//!$this->process_payment->processInitialPayment($creditCardDetails)) {
