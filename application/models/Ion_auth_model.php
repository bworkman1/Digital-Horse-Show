...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, user_image, first_name, last_name, profile_name')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
	...
	**/
	...
	public function set_session($user)
	{
		...
		if(in_array('admin', $userGroups)) {
			$userType = 'admin';
		} elseif(in_array('coach', $userGroups)) {
			$userType = 'coach';
		} else {
			$userType = 'user';
		}
	...
	public function login_remembered_user()
	{
		...
		// get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, email, last_login, user_image, first_name, last_name, profile_name')
